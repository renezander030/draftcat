...
- **Tool-call gate** — `POST /gate/tool-call` puts an agent's individual MCP or SDK tool calls through the same gate as a pipeline step: allowlist, risk tier, human approval, audit row. Denies by default, and the approval is bound to a hash of the exact arguments proposed.
- **Risk tiers** — a step declares `risk: low | normal | high`, and `approval_policy` lets an operator decide in advance that a declared class needs no fresh tap. High risk is never eligible. Every exemption lands in the audit trail as `policy_approve` with the rule that fired, so a policy release is never mistaken for a human decision.
- **Spend shown at the decision** — the approval prompt carries what the run has already spent against its caps, so the person releasing the action sees the number rather than learning it afterwards.
- **Escalation before timeout** — `escalate_after` re-notifies the channel while a gate is still open, and `escalate_to` widens who is told. It never widens who may decide.
